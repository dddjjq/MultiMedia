...
public class MusicListActivity extends BaseActivity implements OnItemFocusListener
    ...
    public void handleUpdate(String type) {
        if (type.equals(Constants.ALL_REFRESH_ACTION)
                ...
                || type.equals(Constants.MUSIC_REFRESH_ACTION)) {
            updateDataList(null);
        }
    }
    ...
    private void updateDataList(final String fromSearch) {
        mExecutorService.execute(new Runnable() {
            @Override
            public void run() {
                final List<MusicData> musicDataList = FileDataManager.getInstance(MusicListActivity.this).getAllMusicData();
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        mDataList.clear();
                        setAdapterAndNotify();
                        mDataList.addAll(musicDataList);
                        if (fromSearch == null) {
                            refreshList();
                        } else {
